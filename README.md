Secure Vault System with Off-Chain Authorization
...
This design improves security, prevents replay attacks, and follows clean smart contract architecture principles.
...
First, off-chain, the admin signs a message containing the vault address, recipient address, withdrawal amount, and a unique authorization ID.

Next, the user submits this signed message to the SecureVault contract.

The SecureVault forwards the data to the AuthorizationManager.

The AuthorizationManager reconstructs the message hash, verifies the signature, and checks if the authorization ID was already used.

If everything is valid, the SecureVault releases the funds to the recipient.

Replay Protection

Replay attacks are prevented using a unique authorization ID for every withdrawal.

The AuthorizationManager maintains a mapping of used authorization IDs.

If an authorization ID has already been used, the transaction is rejected.

Each authorization can be used exactly once, even if the signature is valid.

This ensures strong protection against duplicate withdrawals.
